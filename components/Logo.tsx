...
const Logo: React.FC<{ size?: 'sm' | 'md' | 'lg' }> = ({ size = 'md' }) => {
  ...
  return (
    <div className={`flex items-center ${containerSizes[size]} group cursor-pointer select-none`}>
      <div className={`${markSizes[size]} bg-slate-950 flex items-center justify-center shadow-[0_15px_40px_-5px_rgba(79,70,229,0.5)] group-hover:shadow-[0_20px_50px_-5px_rgba(34,211,238,0.4)] group-hover:-translate-y-1 transition-all duration-700 relative overflow-hidden ring-2 ring-white/30`}>
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 via-purple-600 to-cyan-400 opacity-100 animate-gradient-slow"></div>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.4),transparent)]"></div>
        {/* Interlocking Z-Rail Logo */}
        <svg width="60%" height="60%" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" className="relative z-10 drop-shadow-2xl">
          ...
          <path d="M8 8h4" opacity="0.5" />
          <path d="M12 16h4" opacity="0.5" />
        </svg>
      </div>
      <div className="flex flex-col">
        <h1 className={`${textSizes[size]} font-[1000] tracking-[-0.05em] text-black leading-none`}>
          Zync<span className="text-cyan-500">tra</span>
        </h1>
        <div className="flex items-center gap-2 mt-1.5">
           <span className="text-[10px] font-[1000] text-indigo-700 tracking-[0.4em] uppercase">Pro Hub</span>
