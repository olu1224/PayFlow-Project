...
const TopBar: React.FC<TopBarProps> = ({ user, activeTab, setActiveTab, onOpenNotifications, onOpenSettings, onUpdateCountry }) => {
  ...
  const NavItem = ({ id, labelKey, icon, hasDropdown }: { id: string; labelKey: string; icon: React.ReactNode; hasDropdown?: boolean }) => {
    const isActive = activeTab === id || (hasDropdown && id === openDropdown);
    const label = t(labelKey, user.country);
    ...
    return (
      <div className="relative h-full flex items-center">
        ...
        >
          <span className={isActive ? 'text-white' : 'text-slate-400 group-hover:text-purple-50 group-hover:text-purple-600'}>{icon}</span>
          ...
          >
            ...
            {id === 'ai' && (
              <>
                ...
                <DropdownLink icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>} label="Image Editor" onClick={() => {setActiveTab('ai-image-edit'); setOpenDropdown(null);}} />
  ...
  const DropdownLink = ({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) => (
    ...
    >
      <span className="text-slate-400 group-hover:text-purple-50 group-hover:text-purple-600">{icon}</span>
      {label}
    </button>
  );
    ...
    <div className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 z-50 sticky top-0 shadow-sm">
      <div className="flex items-center gap-6 h-full">
        ...
        <div className="flex items-center gap-1 h-full">
          <NavItem id="dashboard" labelKey="home" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>} />
          <NavItem id="recurring" labelKey="wallet" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 7V4a1 1 0 0 0-1-1H5a2 2 0 0 0 0 4h15a1 1 0 0 1 1 1v4h-3a2 2 0 0 0 0 4h3a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1"/><path d="M3 5v14a2 2 0 0 0 2 2h15a1 1 0 0 0 1-1v-4"/></svg>} />
          <NavItem id="history" labelKey="history" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="14" x="3" y="5" rx="2"/><path d="M12 9v6"/><path d="M9 12h6"/></svg>} />
          <NavItem id="finance" labelKey="finance" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/></svg>} hasDropdown />
          <NavItem id="invest" labelKey="invest" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21.21 15.89A10 10 0 1 1 8 2.83"/><path d="M22 12A10 10 0 0 0 12 2v10z"/></svg>} hasDropdown />
          <NavItem id="ai" labelKey="ai" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/></svg>} hasDropdown />
          <NavItem id="beneficiaries" labelKey="contacts" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><circle cx="19" cy="11" r="2"/></svg>} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-100 hover:bg-slate-50 transition-all font-bold text-sm"
          >
          ...
          {showCountryPicker && (
            <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              {countries.map(c => (
                <button 
                  key={c.name}
                  onClick={() => {
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-bold transition-colors ${
                    user.country === c.name ? 'bg-purple-50 text-purple-600' : 'text-slate-600 hover:bg-slate-50'
                  }`}
                >
