...
const AboutPage: React.FC<{ user: User }> = ({ user }) => {
  return (
    <div className="max-w-[1000px] mx-auto space-y-16 animate-in fade-in duration-700 pb-24 px-4">
      <header className="text-center space-y-6 pt-10">
        <div className="flex justify-center mb-6">
          <Logo size="lg" />
        </div>
        <h1 className="text-4xl md:text-6xl font-[1000] text-slate-900 tracking-tighter">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          { 
            ...
            icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
            color: "bg-purple-600"
          },
          { 
            ...
            icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><circle cx="19" cy="11" r="2"/></svg>,
          ...
          { 
            ...
            icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>,
            color: "bg-amber-500"
          }
        ].map((item, i) => (
          <div key={i} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl space-y-6 group hover:-translate-y-2 transition-all">
            <div className={`w-16 h-16 ${item.color} text-white rounded-2xl flex items-center justify-center shadow-lg group-hover:rotate-6 transition-all`}>
              {item.icon}
            </div>
      ...
      <section className="bg-slate-950 p-10 md:p-20 rounded-[4rem] text-white relative overflow-hidden shadow-2xl">
        <div className="absolute top-0 right-0 w-96 h-96 bg-purple-600 rounded-full blur-[150px] opacity-20"></div>
        <div className="relative z-10 space-y-10">
          <div className="space-y-4">
            <h2 className="text-[10px] font-black text-purple-400 uppercase tracking-[0.4em]">{t('mission_statement', user.country)}</h2>
