...
const App: React.FC = () => {
  // DEVELOPMENT BYPASS: Initialize with a default user session if none exists
  const [user, setUser] = useState<User>(() => {
    ...
    localStorage.setItem('payflow_user_session', JSON.stringify(devUser));
    return devUser;
  });

  // DEVELOPMENT BYPASS: Always unlocked
  ...
  const handleLogout = () => {
    // In dev mode, we just clear and reload with the default
    localStorage.clear();
  ...
  const handleNewTransaction = (amount: number, name: string, category: string = 'General', isRecurring: boolean = false, schedule?: any) => {
    ...
    setUser(prev => ({ ...prev, balance: prev.balance - (isDebit ? absoluteAmount : -absoluteAmount) }));
  };
  ...
  const handleTrade = (assetId: string, amount: number, priceUsd: number, isBuy: boolean) => {
    ...
    setUser(prev => ({ ...prev, balance: isBuy ? prev.balance - totalLocal : prev.balance + totalLocal }));
  ...
  const handleUpgrade = (plan: string) => {
    const updated = { ...user, creditScore: 800 }; 
    setUser(updated);
    localStorage.setItem('payflow_user_session', JSON.stringify(updated));
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard': return <Dashboard user={user} transactions={transactions} onNewTransaction={handleNewTransaction} onDeposit={(amt) => setUser({...user, balance: user.balance + amt})} onWithdraw={(amt) => setUser({...user, balance: user.balance - amt})} onExplorePlanning={() => setActiveTab('money-hub')} onNearbyClick={() => setActiveTab('nearby')} />;
      ...
      default: return <Dashboard user={user} transactions={transactions} onNewTransaction={handleNewTransaction} onDeposit={(amt) => setUser({...user, balance: user.balance + amt})} onWithdraw={(amt) => setUser({...user, balance: user.balance - amt})} onExplorePlanning={() => setActiveTab('money-hub')} onNearbyClick={() => setActiveTab('nearby')} />;
    }
  };
